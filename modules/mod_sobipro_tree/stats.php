...
/**
 * ModSobiproTreeStatsHelper Helper class.
 *
 * @package     Prieco.Modules
 * @subpackage  mod_sobipro_tree
 * @since       1.0
 */
class ModSobiproTreeStatsHelper
{
	...
	public function calculateTotalEntries($section, $debug = 0)
	{
		...
		if ($paths)
		{
			...
			for ($i = 1; ($i <= 5); $i++)
			{
				$query = 'CREATE TEMPORARY TABLE IF NOT EXISTS ' . $tmptablename . $i . ' SELECT * FROM ' . $tmptablename;
				$_db->setQuery($query);
				$_db->query();
				if ($_db->getErrorNum())
				{
		...
		{
			echo 'ERROR: NO Section in Search Plugin+ (Plus).';
		}
