...
/**
 * ModSobiproTreeHelper Helper class.
 *
 * @package     Extly.Modules
 * @subpackage  mod_sobipro_tree
 * @since       1.0
 */
class ModSobiproTreeHelper
{
	...
	public function generate(
		&$list, $iterator_sid,
		$actual_sid,
		$scitemid, $subItemsid, $defaultItemid,
		$scounter, $hide_empty,
		$categorystartlevel, $categorymode, $level = 1, $i = 0, $z=0)
	{
		$html_output = '';
		...
		foreach ($items as $item)
		{
			...
			$isParent = (($childs) && ($level < $categorymode));

			if ($isParent)
			{
					if ($z) $output .= '</div></div></div><div class="accordion-group">';
					else 
					{
					...
					$output .= "<a class=\"accordion-toggle {$class}\" data-toggle=\"collapse\" data-parent=\"#accordion-categories\" href=\"#collapse{$level}{$i}\">{$item->name}</a>";
					$output .="</div>";
...
";
					...
					$output .= 	self::generate(
									...
									$actual_sid,
									$scitemid, $subItemsid, $defaultItemid,
									$scounter, $hide_empty, $categorystartlevel, $categorymode, $level + 1, $i, $z);
			}
			else {
				$output .= "<a href=\"{$url}\" class=\"{$class}\">{$item->name}</a>{$outpcount}<br/>";
			}
		}
