				...
				function setSmallStyle() {
						...
						tabpanes.each(function (i) {
							...
							$(this).swiperight(function() { 
								...
								if (i>0) {
								var prevLi = $(this).parent().siblings("ul").first().children("li:nth-child("+(i)+")"),
								prevTab = $(prevLi).children("a[href]");
								slideTab(thisLi, "out", "right");
								slideTab(prevLi, "in", "left");
								$(prevTab).tab('show');
							...
							$(this).swipeleft(function() { 
								if (i+1<tabpanes.length) {
								var nextLi  = $(this).parent().siblings("ul").first().children("li:nth-child("+(i+2)+")"),
								nextTab = $(nextLi).children("a[href]");
								slideTab(thisLi, "out", "left" );
								slideTab(nextLi, "in",  "right");
								$(nextTab).tab('show');
